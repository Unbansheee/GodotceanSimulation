...
layout (set = 0, binding = 20, r32f) uniform writeonly image2D initial_spectrum;
...
void main() {
    ...
    if (k >= u.cascade_min && k <= u.cascade_min) {
        float U10 = length(u.wind_dir);
        ...
        float cosPhi = dot(normalize(u.wind_dir), normalize(wave_vector));
    ...
    imageStore(initial_spectrum, pixel_coord, vec4(h, 0.f, 0.f, 0.f));
}
